...
Post.findByAuthorId = function(authourId) {
    return Post.reusablePostQuery([
        {$match: {author: authourId}},
        {$sort: {createdDate: -1}}
    ])
}
...
Post.search = function(searchTerm) {
    return new Promise(async (resolve, reject) => {
        if (typeof(searchTerm) == "string") {       //prevent malicious user from doing noSQL injection attack
            let posts = await Post.reusablePostQuery([
...
Post.getFeed = async function(id) {
    ...
    return Post.reusablePostQuery([
        {$match: {author: {$in: followedUsers}}},    //find any post documnet where the author value is in the array of followed users
        {$sort: {createdDate: -1}}
    ])
}
